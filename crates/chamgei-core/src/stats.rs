//! Usage statistics tracking for Chamgei voice dictation.
//!
//! Tracks dictation counts, latency averages, estimated costs, and per-provider
//! usage. Stats are persisted to `~/.config/chamgei/stats.json` so they survive
//! across sessions.
...
/// Returns the path to the stats JSON file: `~/.config/chamgei/stats.json`.
...
/// Aggregated usage statistics for the Chamgei dictation system.
...
impl UsageStats {
    ...
    /// Save stats to `~/.config/chamgei/stats.json`.
    pub fn save(&self) {
        let Some(path) = stats_file_path() else {
