...
impl Dictionary {
    ...
    /// Return the default dictionary file path: `~/.config/chamgei/dictionary.toml`.
...
mod tests {
    ...
    fn inject_vocabulary_with_terms() {
        ...
        assert!(result.contains("Chamgei"));
    ...
    fn load_creates_missing_file() {
        let dir = std::env::temp_dir().join("chamgei-test-dict");
