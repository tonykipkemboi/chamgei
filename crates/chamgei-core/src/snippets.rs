...
//!
//! Snippets are persisted as a TOML file at `~/.config/chamgei/snippets.toml`.
...
impl SnippetStore {
    ...
    /// path (`~/.config/chamgei/snippets.toml`).
    pub fn new() -> Self {
        Self {
...
/// Return the default snippets file path: `~/.config/chamgei/snippets.toml`.
...
"#;
    ...
    fn load_missing_file_is_ok() {
        let mut store = SnippetStore::with_path(PathBuf::from("/tmp/does_not_exist_chamgei.toml"));
