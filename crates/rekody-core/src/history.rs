//! Transcription history persistence for rekody voice dictation.
//!
//! Saves all dictation entries to `~/.config/rekody/history.json` so users
...
/// Returns the path to the history JSON file: `~/.config/rekody/history.json`.
...
impl History {
    /// Load history from `~/.config/rekody/history.json`.
    ///
    ...
    /// Save history to `~/.config/rekody/history.json`.
    pub fn save(&self) {
        let Some(path) = history_file_path() else {
